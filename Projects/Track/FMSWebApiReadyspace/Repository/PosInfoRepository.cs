    ...
    public class PosInfoRepository : IPosRepository
    {
        ...
        public PosInfo Add(PosInfo item)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                ...
                //Logger.LogEvent(mProjName,string.Format("Before Save: Timestamp: {0} PosX: {1} PosY: {2}", item.Timestamp.ToUniversalTime(), item.PosX, item.PosY), System.Diagnostics.EventLogEntryType.Warning);
                ...
                if (item.PosY != 0 && item.PosX != 0)
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //Logger.LogToFile(DateTime.Now + "\t" + String.Format("Duplicated Previous Data - AssetID: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} Last Timestamp: {5} Current Timestamp: {6} Current RxTime: {7}",
                                // currAsset.AssetID, item.PosY, item.PosX, item.Tag, item.Location, udtPrev.Timestamp, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime()), DateTime.Now.ToString("ddMMMyy") + "-DuplicateDataLog.txt");
                ...
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            //Logger.LogEvent(string.Format("Address: {0} Lat: {1} Long: {2}", retVal, lat, lon), System.Diagnostics.EventLogEntryType.Information);
                        }
                        conn.Close();
                    }
                }
