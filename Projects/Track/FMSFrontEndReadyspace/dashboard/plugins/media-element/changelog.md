...
* The project is now MIT-only, instead of dual licensed MIT and GPL (just as jQuery has done: https://jquery.org/license/)
...
* Added horizontal volume control, the new default for audio (based on work by @gavinlynch(https://github.com/gavinlynch))
...
* Integration with YouTube API (and intial support for Vimeo API) : https://mediaelementjs.com/examples/?name=youtube
...
* added hooks for other jQuery compatible libraries like @ender.js(https://enderjs.com)
...
* added captioning support via the `<track>` tag (thanks to @Playr(https://www.delphiki.com/html5/playr) for the example)
