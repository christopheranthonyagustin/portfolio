...
namespace FMSWebApi.Controllers
{
    ...
    public class AssetInfoController : ApiController
    {
        ...
        public IEnumerable<AssetInfo> GetAssetsEx([FromUri]UserInfo param)
        {
            ...
            if ((param.UserID > 0 || param.ResellerID > 0 || param.CompanyID > 0 || !string.IsNullOrEmpty(param.Assets)))
            {
                //Logger.LogEvent(string.Format("Enter GetAssetsEx: {0},{1},{2},{3}",
                // param.UserID, param.ResellerID, param.CompanyID, param.Assets), System.Diagnostics.EventLogEntryType.Information);
                return repository.GetAssetsEx(param);
            }
            else
            {
                //Logger.LogEvent(string.Format("Enter GetAll: {0},{1},{2},{3}",
                // param.UserID, param.ResellerID, param.CompanyID, param.Assets), System.Diagnostics.EventLogEntryType.Information);
                return repository.GetAll();
            }
        }
        ...
        public bool PutAsset(long id, [FromBody]AssetInfo currAsset)
        {
