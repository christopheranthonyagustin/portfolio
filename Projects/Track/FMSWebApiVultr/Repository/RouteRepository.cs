...
namespace FMSWebApi.Repository
{
    public class RouteRepository : IRouteRepository
    {
        ...
        public bool UpdateRoute(RouteInfo currRoute)
        {
            bool retVal = false;
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "UPDATE routes SET driver_id = @DriverID, flag = @Flag WHERE route_id = @RouteID";
                ...
                {
                    Logger.LogEvent(ex.Message + "-UpdateRoute(RouteRepository)", System.Diagnostics.EventLogEntryType.Error);
                }
            }
