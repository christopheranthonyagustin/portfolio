...
namespace FMSWebApi.Repository
{
    public class ZoneRepository : IZoneRepository
    {
        ...
        public IEnumerable<ZoneInfo> GetByCompanyFilter(ZoneInfo param)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<ZoneInfo> GetAssetZones(long iAssetID, int iCompanyID)
        {
            List<ZoneInfo> arrZones = new List<ZoneInfo>();
            ZoneInfo currZone = new ZoneInfo();
            string query = string.Format("SELECT * FROM view_zones WHERE company_id = {0} and perimeter like '% %' ORDER BY name", iCompanyID);
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        {
                            currZone = DataMgrTools.BuildZone(reader);
                            if (currZone.Perimeter.Contains(" "))
                            {
                                arrZones.Add(currZone);
                            }
