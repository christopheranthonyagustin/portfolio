...
namespace FMSWebApi.Repository
{
    public class AssetRepository : IAssetRepository
    {
        ...
        public IEnumerable<AssetInfo> ClearAll()
        {
        ...
        //                    string strFill = "";
        //                    currAsset.Image = GetImage(String.Format("make/{0}", currAsset.MakeID), ref strFill);
        ...
        public AssetInfo GetByName(string assetName, int CompanyID)
        {
            AssetInfo currAsset = new AssetInfo();

            if (string.IsNullOrEmpty(assetName))
            {
                currAsset.ErrorMessage = Consts.ERR_ASSETERROR;
                return currAsset;
            }

            string query = string.Format("SELECT * FROM view_assets WHERE name = @Name and company_id = @CompanyID");
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Name", assetName.Trim());
                ...
                {
                    Logger.LogEvent(ex.Message + "-GetByName(AssetRepository)", System.Diagnostics.EventLogEntryType.Error);
                }
            }
        ...
        public AssetInfo Get(long assetID)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                ...
                {
                    Logger.LogEvent("Get GetAll-view_driver: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                }
        ...
        public IEnumerable<AssetInfo> GetAssetByName(AssetInfo param)
        {

            List<AssetInfo> arrAssets = new List<AssetInfo>();
            AssetInfo currAsset = new AssetInfo();
            string query = string.Format("SELECT * FROM view_assets WHERE name = @Name");
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                ...
                {
                    Logger.LogEvent(ex.Message + "-GetAssetByName", System.Diagnostics.EventLogEntryType.Error);
                }
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.CommandText = "UPDATE assets SET driver_id = @DriverID, asset_remarks = @AssetRemarks " +
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "UPDATE assets SET driver_id = @DriverID, asset_remarks = @AssetRemarks, token = @Token " +
                                          "WHERE asset_id = @AssetID";
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        //get tag id
                        long tagId = RepoHelpers.GetTagId(currAsset.Tag);
                        ...
                        cmd.CommandText = "INSERT INTO assets (name, category_id, tag_id, phone, email, home, restricted, " +
                                                              ...
                                                              "catalog_value, residual_value, seat_no, door_no, color, co2, horsepower, horsepower_taxation, power, features, " +
                                                              "weight, width, height, length, volume, distance) " + 
                                                      "VALUES (@Name, @CategoryID, @TagID, @Phone, @Email, @Home, @AlertZones, " +
                                                              ...
                                                              "@CatalogValue, @ResidualValue, @SeatNo, @DoorNo, @Color, @CO2, @Horsepower, @HorsepowerTaxation, @Power, @Features, " +
                                                              "@Weight, @Width, @Height, @Length, @Volume, @Distance)";
                        ...
                        cmd.Parameters.AddWithValue("@Features", currAsset.Features);
                        cmd.Parameters.AddWithValue("@Weight", currAsset.Weight);
                        cmd.Parameters.AddWithValue("@Width", currAsset.Width);
                        cmd.Parameters.AddWithValue("@Height", currAsset.Height);
                        cmd.Parameters.AddWithValue("@Length", currAsset.Length);
                        cmd.Parameters.AddWithValue("@Volume", currAsset.Volume);
                        cmd.Parameters.AddWithValue("@Distance", currAsset.Distance);
                        cmd.ExecuteNonQuery();
        ...
        public bool Remove(long assetID) 
        {
            bool retVal = false;
                ...
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.CommandText = "UPDATE assets SET name = @Name, category_id = @CategoryID, tag_id = @TagID, phone = @Phone, email = @Email, home = @Home, restricted = @AlertZones, " +
                                                            ...
                                                            "catalog_value = @CatalogValue, residual_value = @ResidualValue, seat_no = @SeatNo, door_no = @DoorNo, color = @Color, co2 = @CO2, horsepower = @Horsepower, horsepower_taxation = @HorsepowerTaxation, power = @Power, features = @Features, " +
                                                            "weight = @Weight, width = @Width, height = @Height, length = @Length, volume = @Volume, distance = @Distance WHERE asset_id = @AssetID";
        ...
        public void SaveAssetInfo(Enums.PosInfoType posTypeID, long iPosID, long iAssetID, DateTime dtTimestamp)
        {
