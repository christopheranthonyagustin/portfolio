...
namespace FMSWebApi.Repository
{
    public class CategoryRepository : ICategoryRepository
    {
        ...
        public IEnumerable<CategoryInfo> GetAll()
        {
            ...
            List<CategoryInfo> arrCategories = new List<CategoryInfo>();
            ...
            try
            {
                ...
                if ((reader != null) && (reader.HasRows))
                {
                    while (reader.Read())
                    {
                        CategoryInfo currCategory = DataMgrTools.BuildCategory(reader);
                        arrCategories.Add(currCategory);
                    }
                }
                conn.Close();
            }
        ...
        public CategoryInfo Add(CategoryInfo currCategory)
        {
            ...
            {
                Logger.LogEvent(ex.Message + "-Add(CategoryRepository)", System.Diagnostics.EventLogEntryType.Error);
            }
        ...
        public bool Remove(int categoryID)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                ...
                {
                    Logger.LogEvent(ex.Message + "-Delete(CategoryRepository)", System.Diagnostics.EventLogEntryType.Error);
                }
            }
        ...
        public bool Update(CategoryInfo currCategory)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "UPDATE categories SET category_desc = @CategoryDesc " +
            ...
            {
                Logger.LogEvent(ex.Message + "-Update(CategoryRepository)", System.Diagnostics.EventLogEntryType.Error);
            }
