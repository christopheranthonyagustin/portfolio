...
namespace FMSWebApi.Repository
{
    public class PosInfoRepository : IPosRepository
    {
        ...
        public IEnumerable<PosInfo> Get([FromUri]PosInfo value)
        {
            ...
            if (value.CompanyID == 22) query += " and fix != 4";
        ...
        public PosInfo Add(PosInfo item)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                ...
                {
                    GetLocation(currAsset, item, null);
                    //if (item.Location.Length == 0 || item.Location.Equals("Request exceeded")) VehTools.ApplyReverseGeocode(currAsset, item, null);
                    //if (item.Location.Length == 0 || item.Location.Equals("Request exceeded")) VehTools.ApplyOneMapReverseGeocode(currAsset, item, null);
                    if (item.Location.Length == 0 || item.Location.Equals("Singapore"))
                    {
                        VehTools.ApplyOneMapReverseGeocode(currAsset, item, null);

                        if (item.Location.Equals("ZERO_RESULTS")) VehTools.ApplyReverseGeocode(currAsset, item, null);
                        //VehTools.ApplyReverseGeocode(currAsset, item, null);
                    }

                    if (item.Location.Equals("Soul Buoy")) item.Location = "No Geocoding Result";
                }
                ...
                try
                {
                    ...
                    item.Driver = currDriver.LoginName;
                }
                catch (Exception ex)
                {
                    Logger.LogEvent("Get Asset Drivers Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                }
                ...
                {
                    Logger.LogEvent("Get Company Zones Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                }
                ...
                if (udtPrev != null)
                {
                    ...
                    //Compute Mileage manually by using Haversine Formula
                    item.Mileage = ComputeMileage(item, udtPrev);
                }
                ...
                //update mileage
                //item.Mileage /= 100;
                //if (item.Mileage > udtPrev.Mileage)
                //    item.Mileage += (item.Mileage - udtPrev.Mileage);
                ...
                //Logger.LogToFile(DateTime.Now + "\t" + String.Format("AssetID: {0}  Zone ID: {1}  Zone: {2}  Device: {3}  Location: {4}", currAsset.AssetID, udtPrev.ZoneID, udtPrev.Zone, item.Tag, item.Location), "debug.txt");
                //Logger.LogToFile(DateTime.Now + "\t" + String.Format("Current Zone ID: {0} Zone: {1}", item.ZoneID, item.Zone), "debug.txt");
                //Logger.LogToFile(DateTime.Now + "\t" + String.Format("iPrevZone: {0}", iPrevZone), "debug.txt");
                int iBuffer = 0;

                switch (item.IsBuffer)
                {
                    case false: iBuffer = 0; break;
                    case true: iBuffer = 1; break;
                    default: iBuffer = 0; break;
                }
                ...
                if (item.PosY != 0 && item.PosX != 0)
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        if (CheckDuplicatePosition(item, currAsset.AssetID) == true)
                        {
                            if (item.PosY == udtPrev.PosY && item.PosX == udtPrev.PosX)
                            {
                                //if (currAsset.AssetID == 227)
                                //{
                                //    Logger.LogToFile(DateTime.Now + "\t" + String.Format("Check position if already in database - AssetID: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} Last Timestamp: {5} Current Timestamp: {6} Current RxTime: {7}",
                                //    currAsset.AssetID, item.PosY, item.PosX, item.Tag, item.Location, udtPrev.Timestamp, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime()), DateTime.Now.ToString("ddMMMyy") + "- Already_in_Data.txt");
                                //}
                                ...
                                cmd.CommandText = "INSERT INTO positions (asset_id, tag_id, timestamp, rx_time, fix, " +
                                                ...
                                                "loc, loc_id, zone_id, remarks, engine, driver, isbuffer) VALUES " +
                                ...
                                cmd.Parameters.AddWithValue("@Timestamp", item.RxTime.ToUniversalTime());
                                ...
                                cmd.Parameters.AddWithValue("@Driver", currDriver.LoginName);
                                cmd.Parameters.AddWithValue("@IsBuffer", iBuffer);
                                cmd.ExecuteNonQuery();
                                ...
                                {
                                    Logger.LogEvent("UPDATE LastPos Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                                }
                            ...
                            else
                            {
                                if (currAsset.AssetID == 227)
                                {
                                    Logger.LogToFile(DateTime.Now + "\t" + String.Format("Duplicated Data - AssetID: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} GPS Timestamp: {5} Current Timestamp: {6}", currAsset.AssetID, item.PosY, item.PosX, item.Tag, item.Location, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime()), DateTime.Now.ToString("ddMMMyy") + "-DuplicateDataLog.txt");
                                }
                                conn.Open();
                                cmd.Connection = conn;
                                cmd.CommandText = "INSERT INTO positions (asset_id, tag_id, timestamp, rx_time, fix, " +
                                ...
                                cmd.Parameters.AddWithValue("@Timestamp", item.RxTime.ToUniversalTime());
                                cmd.Parameters.AddWithValue("@RxTime", item.RxTime);
                                cmd.Parameters.AddWithValue("@FixID", 0);
                                cmd.Parameters.AddWithValue("@Satellites", item.Satellites);
                                cmd.Parameters.AddWithValue("@PosX", item.PosX);
                                cmd.Parameters.AddWithValue("@PosY", item.PosY);
                                cmd.Parameters.AddWithValue("@PosZ", item.PosZ);
                        ...
                        {
                            //Check Previous Data
                            if (item.Timestamp.ToUniversalTime() == udtPrev.Timestamp && item.PosY == udtPrev.PosY && item.PosX == udtPrev.PosX)
                            {

                                //if (currAsset.AssetID == 227)
                                //{
                                //    Logger.LogToFile(DateTime.Now + "\t" + String.Format("Check Previous Data - AssetID: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} Last Timestamp: {5} Current Timestamp: {6} Current RxTime: {7}",
                                //    currAsset.AssetID, item.PosY, item.PosX, item.Tag, item.Location, udtPrev.Timestamp, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime()), DateTime.Now.ToString("ddMMMyy") + "-Check_Previous_Data.txt");
                                //}
                            ...
                            {
                                //if (currAsset.AssetID == 221)
                                //{
                                //    Logger.LogToFile(DateTime.Now + "\t" + String.Format("Valid Data - AssetID: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} Last Timestamp: {5} Current Timestamp: {6} Current RxTime: {7} Ignition: {8}",
                                //    currAsset.AssetID, item.PosY, item.PosX, item.Tag, item.Location, udtPrev.Timestamp, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime(), item.Ignition), DateTime.Now.ToString("ddMMMyy") + "-Valid_Data.txt");
                                //    //Valid Data
                                //}
                                ...
                                if (IsTheSameDay(DateTime.UtcNow, item.Timestamp) == true && item.FixID > 1 && !item.Location.Equals("ZERO_RESULTS"))
                                {
                                    try
                                    {
                                        cmd.CommandText = "UPDATE assets SET last_pos = @LastPosID, last_fix = @LastFixID, last_gps = @LastGpsID WHERE asset_id = @AssetID2";
                                    ...
                                    if ((item.PosID > 0) && (item.Timestamp > udtPrev.Timestamp.ToUniversalTime()))
                                    {
                                        // check if good fix
                                        if (item.FixID != 0)
                                        {
                                            ...
                                            if (item.Speed >= iSpeed)
                                            {
                                                ...
                                                {
                                                    Logger.LogEvent("UPDATE Last Move Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                                                }
                                            }
                                            ...
                                            if ((iLastEngine != 1) && (iEngine == 1))
                                            {
                                                ...
                                                {
                                                    Logger.LogEvent("UPDATE Last Idle Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                                                }
                                            ...
                                            try
                                            {
                                                ...
                                                arrValues.Add(currAsset.AssetID);
                                                cmd.CommandText = String.Format("UPDATE assets SET {0} WHERE asset_id = @AssetID5", strParams);
                                                
                                                cmd.Parameters.AddWithValue("@PosID", item.PosID);
                                                cmd.Parameters.AddWithValue("@AssetID5", currAsset.AssetID);
                                                cmd.ExecuteNonQuery();
                                                arrValues.ToArray();
                                            }
                                            catch (Exception ex)
                                            {
                                                Logger.LogEvent("UPDATE Last Status Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                                            }
                                            ...
                                            */
                                            ...
                                            if (iPrevZone != item.ZoneID)
                                            {
                                                    ...
                                                    foreach (ZoneInfo udtZone in arrZones)
                                                    {
                                                        ...
                                                        if (udtZone.ZoneID == iPrevZone)
                                                        {
                                                            try
                                                            {
                                                                ...
                                                                //if (currAsset.AssetID == 221)
                                                                //{
                                                                //    Logger.LogToFile(DateTime.Now + "\t" + String.Format(" loop through zones - Zone: {0}  Notify: {1}", udtZone.ZoneID, arrAlertZones.Contains(iPrevZone.ToString())), DateTime.Now.ToString("ddMMMyy") + "-Zones.txt");
                                                                //}
                                                            ...
                                                            {
                                                                Logger.LogEvent("Zone Exit Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                                                            }
                                                        }
                                                    }
                                                }
                                                ...
                                                if (item.ZoneID != 0)
                                                {
                                                    ...
                                                    {
                                                        Logger.LogEvent("Zone Enter Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                                                    }
                                                }
                                            }
                                            #endregion
                                            ...
                                            //double lMileage = currAsset.Mileage + (item.Mileage / 100);
                                            //udtEvent.Event = "Mileage Report";
                                            //udtEvent.Remarks = String.Format("{0} km", lMileage);
                                            //udtEvent.AlertLevel = 0;
                                            //udtEvent.Flag = 1;
                                            //udtEvent.CompanyID = currAsset.CompanyID;
                                            //if (currAsset.AssetID == 119) Logger.LogEvent(mProjName, String.Format("MileageDist {0} lMileage: {1}", currAsset.MileageDist, lMileage), System.Diagnostics.EventLogEntryType.Warning);
                                            //// check mileage distance
                                            //if ((currAsset.MileageDist > 0) && (lMileage >= currAsset.MileageDist))
                                            //{
                                            //    // raise mileage alert then reset settings
                                            //    eventRepo.SaveEvent(udtEvent, false, true);

                                            //    cmd.CommandText = String.Format("UPDATE assets SET mileage_dist = {0} WHERE asset_id = {1}", currAsset.MileageDist * -1, currAsset.AssetID);
                                            //    
                                            //    cmd.ExecuteNonQuery();
                                            //}

                                            //// check mileage date
                                            //else if (currAsset.MileageDate > DateTime.MinValue)
                                            //{
                                            //    // check if date for mileage reminder has passed
                                            //    TimeSpan tsDiff = item.Timestamp - currAsset.MileageDate;
                                            ...
                                            //// check idling limit
                                            //if (currAsset.SpeedLimit > 0)
                                            //{
                                            //    udtEvent.Event = "Speeding Report";
                                            //    if (iEngine == 2)
                                            //    {
                                            //        // check idling limit
                                            //        if (item.Speed > currAsset.SpeedLimit)
                                            //        {
                                            //            // raise speeding alert                                       
                                            //            udtEvent.Remarks = String.Format("Speed: {0}kph > Driving Limit", item.Speed);
                                            //            udtEvent.AlertLevel = 1;
                                            //            udtEvent.Flag = 1;
                                            //            udtEvent.CompanyID = currAsset.CompanyID;
                                            //            eventRepo.SaveEvent(udtEvent);
                                            //        }
                                            //    }
                                            //    else
                                            //    {
                                ...
                                else if (IsTheSameDay(DateTime.UtcNow, item.Timestamp.ToUniversalTime()) == true)
                                {
                ...
                else
                {
                    //if (currAsset.AssetID == 227)
                    //{
                    //    Logger.LogToFile(DateTime.Now + "\t" + String.Format("Update Zero Position - AssetID: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} Last Timestamp: {5} Current Timestamp: {6} Current RxTime: {7} Ignition: {8}",
                    //    currAsset.AssetID, item.PosY, item.PosX, item.Tag, item.Location, udtPrev.Timestamp, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime(), item.Ignition), DateTime.Now.ToString("ddMMMyy") + "-Zero_Position.txt");
                    //}
                    if (!string.IsNullOrEmpty(item.Tag))
                    {
                        //Logger.LogToFile(DateTime.Now + "\t" + String.Format("Update Zero Position - Asset: {0}  Lat: {1}  Lon: {2}  Device: {3}  Location: {4} Last Timestamp: {5} Current Timestamp: {6} Current RxTime: {7} Ignition: {8}",
                        //currAsset.Name, item.PosY, item.PosX, item.Tag, item.Location, udtPrev.Timestamp, item.Timestamp.ToUniversalTime(), item.RxTime.ToUniversalTime(), item.Ignition), DateTime.Now.ToString("ddMMMyy") + "-Zero_Position.txt");


                        int iFix = 0;

                        switch (udtPrev.Ignition)
                        {
                            case 0: iFix = 4; break;
                            case 1: iFix = 2; break;
                            default: iFix = 2; break;
                        }
                        ...
                        using (MySqlCommand cmd = new MySqlCommand())
                        {
                            ...
                            cmd.Parameters.AddWithValue("@Timestamp", item.RxTime.ToUniversalTime());
                            cmd.Parameters.AddWithValue("@RxTime", item.RxTime);
                            cmd.Parameters.AddWithValue("@FixID", iFix);
                            cmd.Parameters.AddWithValue("@Satellites", item.Satellites);
                            cmd.Parameters.AddWithValue("@PosX", udtPrev.PosX);
                            cmd.Parameters.AddWithValue("@PosY", udtPrev.PosY);
                            cmd.Parameters.AddWithValue("@PosZ", udtPrev.PosZ);
                            cmd.Parameters.AddWithValue("@Speed", udtPrev.Speed);
                            cmd.Parameters.AddWithValue("@Course", udtPrev.Course);
                            cmd.Parameters.AddWithValue("@Hdop", udtPrev.HDOP);
                            cmd.Parameters.AddWithValue("@Ignition", item.Ignition);
                            cmd.Parameters.AddWithValue("@Mileage", udtPrev.Mileage);
                            cmd.Parameters.AddWithValue("@Battery", udtPrev.Battery);
                            cmd.Parameters.AddWithValue("@Fuel", udtPrev.Fuel);
                            cmd.Parameters.AddWithValue("@Location", udtPrev.Location);
                            cmd.Parameters.AddWithValue("@LocID", udtPrev.LocID);
                            cmd.Parameters.AddWithValue("@ZoneID", udtPrev.ZoneID);
                            cmd.Parameters.AddWithValue("@Remarks", udtPrev.Remarks);
                            cmd.Parameters.AddWithValue("@Engine", iEngine);
                            cmd.Parameters.AddWithValue("@Driver", currDriver.LoginName);
                            cmd.Parameters.AddWithValue("@IsBuffer", iBuffer);
                            cmd.ExecuteNonQuery();
        ...
        public PosInfo AddOneMap(PosInfo item)
        {
            AssetInfo currAsset = new AssetInfo();
            DriverInfo currDriver = new DriverInfo();
            EventRepository eventRepo = new EventRepository();
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                ...
                GetLocation(currAsset, item, null);
                //if (item.Location.Length == 0 || item.Location.Equals("Request exceeded")) 
                if (item.Location.Length == 0) VehTools.ApplyOneMapReverseGeocode(currAsset, item, null);
        ...
        private bool CheckDuplicatePosition(PosInfo param, long asset_id)
        {
            bool retVal = false;
            List<PosInfo> arrPos = new List<PosInfo>();
            PosInfo currPos = new PosInfo();
            string strStart = (param.Timestamp != DateTime.MinValue ? String.Format("{0:yyyy-MM-dd HH:mm:ss}", param.Timestamp) : "NULL");


            string query = "SELECT * FROM view_positions WHERE timestamp = @StartTS and asset_id = @AssetID";
            //string query = "SELECT * FROM view_positions WHERE (timestamp = @StartTS and @EndTS) and asset_id = @AssetID";
            //if (param.PosY != 0 && param.PosX != 0) query += " and pos_x = @PosX and pos_y = @PosY";
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@StartTS", strStart);
                        cmd.Parameters.AddWithValue("@EndTS", strStart);
                        //cmd.Parameters.AddWithValue("@PosX", param.PosX);
                        //cmd.Parameters.AddWithValue("@PosY", param.PosY);
                ...
                { 
                    Logger.LogEvent(mProjName, "Check Duplicate Position: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                }
            }
                ...
                {              
                            ...
                            try
                            {
                                ...
                                using (MySqlCommand cmd = new MySqlCommand())
                                {
                                    conn.Open();
                                    cmd.Connection = conn;
                                    cmd.CommandText = "SELECT * FROM utilization WHERE date = @Date AND asset_id = @AssetID order by date desc LIMIT 1";
        ...
        private double ComputeMileage(FMSWebApi.Models.PosInfo udtPos, FMSWebApi.Models.PosInfo udtPrev)
        {
            ...
            {
                Logger.LogEvent(mProjName, "ComputeMileage: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
        ...
        public void GetLocation(AssetInfo udtAsset, PosInfo udtPos, PosInfo udtPrev)
        {
            Enums.Engine currEngine = VehTools.GetEngineStatus(udtPos.Ignition, udtPos.Speed, udtPos.Timestamp, udtAsset.LastMove);
                    ...
                    case Enums.Engine.STOP:
                        {
                            ...
                            udtPos.Location = GetLocArchive(udtPos.PosY, udtPos.PosX);
                            udtPos.LocID = 0;
                  
                            if (udtPos.Location.Length == 0 || udtPos.Location.Equals("") || udtPos.Location.Equals("Atlas Buoy 0.00E 0.00N")) udtPos.Location = "Singapore";
            ...
            {
                Logger.LogEvent("Location Archived ERROR: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
        }
        private string GetLocArchive(double lat, double lon)
        {
            LocationInfo currLocation = new LocationInfo();
            string retVal = "";
            //string query = string.Format("SELECT loc FROM positions where pos_y = @PosY and pos_x = @PosX order by timestamp desc LIMIT 1");
            string query = string.Format("SELECT loc FROM positions where pos_y = @PosY and pos_x = @PosX and loc is not null LIMIT 1");
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.Parameters.AddWithValue("@PosY", lat);
                        cmd.Parameters.AddWithValue("@PosX", lon);
                        MySqlDataReader reader = cmd.ExecuteReader();

                        while (reader.Read())
                        {
                            currLocation = DataMgrTools.BuildLocation(reader);
                            retVal = currLocation.Location;
                        }
                        conn.Close();
                    }
                }
            ...
            {
                Logger.LogEvent(ex.Message + "-GetLocationInfo", System.Diagnostics.EventLogEntryType.Error);
            }
