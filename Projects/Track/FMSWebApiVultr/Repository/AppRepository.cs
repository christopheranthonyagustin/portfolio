            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "SELECT * FROM view_drivers WHERE login_name = @LoginName AND password = @Password";
                        
                        cmd.Parameters.AddWithValue("@LoginName", newInfo.LoginName);
                        ...
                        if ((reader != null) && (reader.HasRows))
                        {
                            while (reader.Read())
                            {
                                ...
                                newInfo.ImageCompanyFill = strFill;
                            }             
                        }
                        else
                        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Settings.Default.DB))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "SELECT COUNT(*) FROM view_drivers WHERE login_name = @LoginName AND password = @Password";
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Settings.Default.DB))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "SELECT * FROM view_drivers WHERE login_name = @LoginName LIMIT 1";
