...
namespace FMSWebApi.Repository
{
    public class PosInfoRepository : IPosRepository
    {
        ...
        private string mConnStr = "server=localhost;uid=root;pwd=$B$Transit888;database=adas;charset=utf8;max pool size=500;default command timeout=99999999;";
        ...
        public IEnumerable<TriggerInfo> GetAllEx()
        {
            MySqlConnection conn = new MySqlConnection();
            MySqlCommand cmd = new MySqlCommand();
            List<TriggerInfo> arrPos = new List<TriggerInfo>();
        ...
        public IEnumerable<TriggerInfo> GetTriggerData([FromUri]TriggerInfo value)
        {
            List<TriggerInfo> arrPos = new List<TriggerInfo>();
            TriggerInfo currPos = new TriggerInfo();
            object objTemp = new object();

            string query = "SELECT * FROM view_trigger WHERE timestamp between @StartTS and @EndTS";
            if (value.CompanyID > 0) query += " and company_id = @CompanyID";
            if (!string.IsNullOrEmpty(value.SvcNo)) query += " and (svc_no IN (" + value.SvcNo + "))";
            if (!string.IsNullOrEmpty(value.Drv)) query += " and (drv_no IN (" + value.Drv + "))";
            query += " and event not IN (77, 84, 83, 68)";

            Logger.LogEvent("Check Trigger Data Query: " + query, System.Diagnostics.EventLogEntryType.Warning);
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if ((reader != null) && (reader.HasRows))
                            {
                                while (reader.Read())
                                {
                                    currPos = DataMgrTools.BuildTrigger(reader);
                                    arrPos.Add(currPos);
                                }
                            }
                        }
        ...
        public IEnumerable<TriggerInfo> PostTrigger([FromBody]TriggerInfo value)
        {
            List<TriggerInfo> arrPos = new List<TriggerInfo>();
            TriggerInfo currPos = new TriggerInfo();
            ...
            if (value.CompanyID > 0) query += " and company_id = @CompanyID";
            if (!string.IsNullOrEmpty(value.SvcNo)) query += " and (svc_no IN (" + value.SvcNo + "))";
            if (!string.IsNullOrEmpty(value.DutyNo)) query += " and (drv_no IN (" + value.DutyNo + "))";
            query += " and event not IN (77, 84, 83, 68)";

            Logger.LogEvent("Check PostTriggerData Query: " + query, System.Diagnostics.EventLogEntryType.Warning);
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
