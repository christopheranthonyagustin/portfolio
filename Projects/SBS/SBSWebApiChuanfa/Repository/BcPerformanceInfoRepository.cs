                            ...
                            " FROM" +
                                " (SELECT bcrank.local_date AS Date, bcrank.drv_no, (count(*) / (svc_mileage.mileage * drvsched.trip)) * 100 AS safety_events" +
                                    " FROM (SELECT" +
                                        ...
                                        " ) AS bcrank" +
                                    " INNER JOIN (SELECT" +
                                            " OP_DATE, SVC_NO, COUNT(*) AS Trip" +
                                        " FROM" +
                                            " tags AS tags" +
                                        " INNER JOIN (SELECT" +
                                            " OP_DATE, DRV_NO, BUS_NO, SVC_NO" +
                                        " FROM" +
                                            " driver_sched_archive" +
                                        " WHERE" +
                                            " OP_DATE between @StartOP1 and @EndOP1" +
                                            " AND DRV_NO IN (" + value.DrvNo1 + ")) AS svcsched ON tags.name = svcsched.BUS_NO" +
                                        " GROUP BY OP_DATE , SVC_NO) AS drvsched ON bcrank.local_date = drvsched.OP_DATE" +
                                            " AND bcrank.svc_no = drvsched.svc_no" +
                                    " INNER JOIN svc_mileage ON bcrank.svc_no = svc_mileage.svc_no" +
                                ...
                                " (SELECT bcrank.local_date AS Date, bcrank.drv_no, (count(*) / (svc_mileage.mileage * drvsched.trip)) * 100 AS safety_events" +
                                    ...
                                    " INNER JOIN (SELECT" +
                                        ...
                                        " FROM" +
                                            " driver_sched_archive" +
                                        " WHERE" +
                                            " OP_DATE between @StartOP2 and @EndOP2" +
                                            " AND DRV_NO IN (" + value.DrvNo2 + ")) AS svcsched ON tags.name = svcsched.BUS_NO" +
                                        " GROUP BY OP_DATE , SVC_NO) AS drvsched ON bcrank.local_date = drvsched.OP_DATE" +
                                            " AND bcrank.svc_no = drvsched.svc_no" +
                                    " INNER JOIN svc_mileage ON bcrank.svc_no = svc_mileage.svc_no" +
                                    " WHERE svc_mileage.direction_id = 1" +
