            ...
            try
            {
                ...
                if (bConnected)
                {
                    ...
                    if (iLen > 0)
                    {
                        ...
                        //Mobile Eye
                        if (strTemp.StartsWith("I") && strTemp.EndsWith("\r"))
                        {
                            //Send Ack
                            ...
                            try
                            {
                                ...
                                string dt = String.Format("T,{0:ddMMyy,HHmmss}", dateTime);
                                Send(dt + "\r", sockInfo.Socket);
                                Logger.LogToEvent("Time Synchronization I: " + dt, System.Diagnostics.EventLogEntryType.Information);//testing
                            }
                            catch (Exception ex)
                            {
                                Logger.LogToEvent("Date I Exception: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                            }
                            ...
                            if (strTempArr.Length > 1)
                            {
                                ...
                                string parseTemp = splitStr.Replace("&,", "\r" + mDeviceIdSBS + ";" + "L");
                                string[] array = parseTemp.Split(',');
                                StringBuilder sb = new StringBuilder();
                                for (int i = 0; i < array.Length - 1; i += 6)
                                {
                                    sb.AppendLine(string.Format("{0},{1}", array[0], string.Join(",", array.Skip(i + 1).Take(6))));
                                }
                                strTemp = String.Format("{0}", sb.ToString());

                                Logger.LogToEvent("If Test I: " + strTemp, System.Diagnostics.EventLogEntryType.Error);//testing
                            }
                            else
                            {
                                string splitStr = string.Format("&,{0}\r", strTemp);
                        ...
                        else
                        {
                           ...
                           // for (int i = 0; i < array.Length - 1; i += 6)
                           // {
                           //     sb.AppendLine(string.Format(mDeviceIdSBS + ";{0},{1}", array[0], string.Join(",", array.Skip(i + 1).Take(6))));
                           // }
                           ...
                           // strTemp = String.Format("{0}", sb.ToString());
                            ...
                            if (strTempArr.Length > 1)
                            {
                                ...
                                Logger.LogToEvent("If Test L: " + strTemp, System.Diagnostics.EventLogEntryType.Error);//testing
                            }
                            else
                            {
                                string splitStr = string.Format("&,{0}\r", strTemp);
                        ...
                        if (strData != "")
                        {
                            ...
                            for (int i = 0; i < arrData.Length; i++)
                            {
                                ...
                                Logger.LogToEvent("This is strData: " + arrData[i], System.Diagnostics.EventLogEntryType.Error);//testing
                                //if (strData.Trim() != "")
