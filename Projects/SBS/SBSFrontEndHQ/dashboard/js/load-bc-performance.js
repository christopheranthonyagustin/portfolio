...
$(document).ready(function () {
    ...
    function Generate() {
        ...
        if ($('#bcPerformance').highcharts() != null) $('#bcPerformance').highcharts().destroy();
        ...
        {
            //http://172.16.10.110/sbswebapi/api/bcperformanceinfo?DepotID=1&InterchangeID=1&DrvNo=15989&Timestamp=31-Aug-2016,%2004:00:00%20PM&RxTime=1-Sep-2016,%2004:00:00%20PM
            //WebApi = "http://172.16.10.110/sbswebapi/api/bcperformanceinfo?DepotID1=" + parseObjDepot + "&DepotID2=" + parseObjDepot2 + "&DrvNo1=" + getDriver + "&DrvNo2=" + getDriver2 + "&Timestamp1=" + timestamp + "&RxTime1=" + rxtime + "&Timestamp2=" + timestamp2 + "&RxTime2=" + rxtime2 + "&Criteria=" + getCriteria;
            //WebApi2 = "http://172.16.10.110/sbswebapi/api/bcperformanceinfo?DepotID=" + parseObjDepot2 + "&DrvNo=" + getDriver2 + "&Timestamp=" + timestamp2 + "&RxTime=" + rxtime2 + "&Criteria=" + getCriteria;
            WebApi = "http://172.16.10.110/sbswebapi/api/bcperformanceinfo?DrvNo1=" + getDriver + "&DrvNo2=" + getDriver2 + "&Timestamp1=" + timestamp + "&RxTime1=" + rxtime + "&Timestamp2=" + timestamp2 + "&RxTime2=" + rxtime2 + "&Criteria=" + getCriteria;
        }
            ...
            if (validateTimestamp == true && validateRxTime == true) {
                ...
                if (getTimestamp < getRxTime) {
                    ...
                    var table = $("#BcPerf").DataTable({
                        ...
                        "columns": [
                            { data: "RangeID", title: "Range ID" },
                            { data: "IndexValue", title: "Index Value" },
                            { data: "RangeCount", title: "Range Count" }
                        ],
                        "ajax": {
                            url: WebApi,
                            type: 'GET',
                        ...
                        "footerCallback": function (row, data, start, end, display) {
                            ...
                            var convertTimestamp = moment(getTimestamp, dateFormat);
                            var convertRxtime = moment(getRxTime, dateFormat);
                            var duration = '<div style="color:#333333">Step 1 Selected Date:&nbsp;' + moment.duration(convertRxtime - convertTimestamp).humanize() + ' duration</div>';

                            document.getElementById("timeIntervalBcPerf1").innerHTML = duration;

                            var getTimestamp2 = $("#dateFrom2").val();
                            var getRxTime2 = $("#dateTo2").val();
                            var convertTimestamp2 = moment(getTimestamp2, dateFormat);
                            var convertRxtime2 = moment(getRxTime2, dateFormat);
                            var duration2 = '<div style="color:#333333">Step 2 Selected Date:&nbsp;' + moment.duration(convertRxtime2 - convertTimestamp2).humanize() + ' duration</div>';

                            document.getElementById("timeIntervalBcPerf2").innerHTML = duration2;
                        }
                    });
...
$(function () {
    ...
    $('.SelectSvcFilter').on('change', function () {
		...
		var getSvc = $('#svc').val().join('\',\''); //svc
		var getSvc = "'" + getSvc + "'"
		//var testSvc = $( "#svc option:selected" ).text();
        ...
        else
        {
            ...
            $('#driver').empty();
            //http://172.16.10.110/sbswebapi/api/scsarchiveinfo?Depot=1,2&Interchange=1,2&Interchange=1&OP_DATE_START=01-Sep-2016&OP_DATE_END=05-Sep-2016 
            var WebAPISvc = "http://172.16.10.110/sbswebapi/api/scsarchiveinfo?Depot=" + getDepot + "&Interchange=" + getInterchange + "&SVC_NO=" + getSvc + "&OP_DATE_START=" + timestamp + "&OP_DATE_END=" + rxtime;
    ...
    $('.SelectSvcFilter2').on('change', function () {
		...
		var getSvc2 = $('#svc2').val().join('\',\''); //svc
		var getSvc2 = "'" + getSvc2 + "'"
        //var ObjSvc2 = JSON.stringify(getSvc2);
        //var parseObjSvc2 = ObjSvc.replace(/\[|\"|\]/g, "\'");
        ...
        if (arrSelected == "" || arrSelected == null) {
            ...
            $('#driver2').empty();
            //http://172.16.10.110/sbswebapi/api/scsarchiveinfo?Depot=1,2&Interchange=1,2&Interchange=1&OP_DATE_START=01-Sep-2016&OP_DATE_END=05-Sep-2016 
            var WebAPISvc = "http://172.16.10.110/sbswebapi/api/scsarchiveinfo?Depot=" + getDepot + "&Interchange=" + getInterchange + "&SVC_NO=" + getSvc2 + "&OP_DATE_START=" + timestamp + "&OP_DATE_END=" + rxtime;
