...
namespace WlocateWeb
{
    public class DatabaseMgr
    {
        ...
        /// database handler constructor
        /// </summary>
        public DatabaseMgr(string strConnection)
        {
            // create database connection
            m_sqlConnection = new SqlConnection(strConnection);
        }
        ...
        public bool Open()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    m_sqlConnection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString2"]);
                    return Open();
                }
            }
            catch (Exception ex)
        ...
        public static string LogError(string strError, string strSource, string strSql)
        {
            ...
            {
                // log event
                string strLog = String.Format("WlocateWeb.{0}: {1}\r\n\"{2}\"", strSource, strError, strSql);
                LogEvent(strLog, 2);
            }
            catch { }
            return strError;
        ...
        public static void LogEvent(string strEvent, int iType)
        {
            try
            {
                ...
                // check error type
                string strLog = String.Format("{0}\\WlocateWeb: {1}", instanceName, strEvent);
                switch (iType)
                {
