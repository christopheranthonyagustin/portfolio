...
$(document).ready(function () {
    ...
    $("#submit-filter").click(function () {
        ...
        $('#pests-editable').bootstrapTable('destroy');

        var WebAPI = "https://webapi.systempest.com/api/pestinfo";

        $('#pests-editable').bootstrapTable({
            idField: 'id',
            url: WebAPI,
            columns: [{
                field: 'state',
            ...
            onCheck: function (row) {
                ...
                var pestID = $('#pestID').val(row.PestID);
                var pestDesc = $('#pestEN').val(row.PestDesc);
                $(".selectpicker").selectpicker('refresh');
...
$(document).ready(function() {
	...
	$('#pests-editable').bootstrapTable({
		...
		onCheck: function (row) {
            ...
            if (row.PestID >= 1 && row.PestID <= 31) {

                $("#toggle-aside").show();
                $("toggle-aside2").hide();
                clearInputField();
                $('#pest-confirm-deleteEN').prop('disabled', true);
                $('#pests-editable').bootstrapTable('refresh');

                $.niftyNoty({
                    type: 'danger',
                    icon: 'fa fa-times',
                    message: 'Default Pests not allowed to edit',
                    container: 'floating',
                    timer: 3000
                });
...
//Delete Row
var $table = $('#pests-editable'), $remove = $('#pest-confirm-delete'), $removeEN = $('#pest-confirm-deleteEN');
...
// =================================================================
function pestsFormatter(value, row) {
	return '<a href="#" class="btn-link" > Pests #' + value + '</a>';
}
...
$(function () {
    ...
    $('.tgl-menu-btn').click(function () {
        ...
        setTimeout(function () {
            $('#pests-editable').bootstrapTable('resetView');
...
if (getSessionstorageValueLanguage == "en") {
    ...
    $("#next").text("Next");
    $("#submit-pestEN").text("Submit");
    $("#resellerName").text("Reseller");
    $("#enableReport").text("Enable Report");
    $("#clickLogoutEN").text("OK");
    $("#pest-confirm-deleteEN").text("Delete");
    $("#PestsExportWordEN").text("Word");
    $("#PestsExportPDFEN").text("PDF");
    $("#PestsExportExcelEN").text("Excel");
