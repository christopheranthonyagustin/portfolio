...
$(document).ready(function () {
    ...
    $("#submit-filter").click(function () {
        ...
        $('#preventions-editable').bootstrapTable('destroy');

        var WebAPI = "https://webapi.systempest.com/api/recommendationsinfo";

        $('#preventions-editable').bootstrapTable({
            idField: 'id',
            url: WebAPI,
            columns: [{
                field: 'state',
                ...
                formatter: 'preventionsFormatter'
            }, {
            ...
            onCheck: function (row) {
                ...
                var preventionID = $('#preventionID').val(row.RecommendationsID);
                var preventionDesc = $('#preventionEN').val(row.Recommendation);
                $(".selectpicker").selectpicker('refresh');
...
$(document).ready(function() {
    ...
    $('#preventions-editable').bootstrapTable({
		...
		onCheck: function (row) {
            ...
            if (row.RecommendationsID >= 1 && row.RecommendationsID <= 10) {

                $("#toggle-aside").show();
                $("toggle-aside2").hide();
                clearInputField();
                $('#prevention-confirm-deleteEN').prop('disabled', true);
                $('#preventions-editable').bootstrapTable('refresh');

                $.niftyNoty({
                    type: 'danger',
                    icon: 'fa fa-times',
                    message: 'Default Prevention Tip not allowed to edit',
                    container: 'floating',
                    timer: 3000
                });
...
//Delete Row
var $table = $('#preventions-editable'), $remove = $('#prevention-confirm-delete'), $removeEN = $('#prevention-confirm-deleteEN');
...
{
    $('#main-wz-company').bootstrapWizard('first');
}
...
// =================================================================
function preventionsFormatter(value, row) {
	return '<a href="#" class="btn-link" > Prevention #' + value + '</a>';
}
...
$(function () {
    ...
    $('.tgl-menu-btn').click(function () {
        ...
        setTimeout(function () {
            $('#preventions-editable').bootstrapTable('resetView');
    ...
    //$('.add-treatmentEN').click(function () {

    //    setTimeout(function () {
    //        $('#treatments-editable').bootstrapTable('resetView');
    ...
    $('.edit-preventionEN').click(function () {
...
if (getSessionstorageValueLanguage == "en") {
    ...
    $("#next").text("Next");
    $("#submit-preventionEN").text("Submit");
    $("#resellerName").text("Reseller");
    $("#enableReport").text("Enable Report");
    $("#clickLogoutEN").text("OK");
    $("#prevention-confirm-deleteEN").text("Delete");
    $("#PreventionsExportWordEN").text("Word");
    $("#PreventionsExportPDFEN").text("PDF");
    $("#PreventionsExportExcelEN").text("Excel");
