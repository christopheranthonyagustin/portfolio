...
$(function () {
    ...
    //Populate phone user
    var WebAPITreatment = "https://webapi.systempest.com/api/treatmentinfo";
...
function serialFormatter(value) {
    ...
    return '<span class="sequence-number">' + value + 1 + '</span>';
}
...
function reloadJobsCreated() {
    ...
    if (getSessionstorageValueRoleID == 1) {
        ...
        WebAPIJobs = 'https://webapi.systempest.com/api/maintenanceinfo?CompanyID=' + getSessionstorageValueCompanyID;
    } else if (getSessionstorageValueRoleID == 2) {
                ...
                for (k = 0; k < res.length; k++) {
                    ...
                    if (res[k] != undefined || res[k]) {
                        ...
                        outputDiv.innerHTML += "<table id='jobsCreated' border='1'>" +
                            ...
                            "&nbsp;&nbsp;&nbsp;&nbsp;<strong><a href='javascript:deletejob(" + JSON.stringify(res[k]) + ")' style='color:#458FD2;'>" + "Delete" + '</a></strong>' +
                            "</td>" +
                            "</tr>" +
                            "</tbody>"; +
                        "</table>";
...
function searchJobFilter() {
    ...
    var convertTimestamp = moment(start, dateFormat);
    var convertRxtime = moment(end, dateFormat);
    ...
    WebAPIJobs = 'https://webapi.systempest.com/api/maintenancefilterinfo?CompanyID=' + getSessionstorageValueCompanyID + '&CusCompany=' + companyName + '&PIC=' + personInCharge + '&Postal=' + postalCode + '&Address=' + address + '&Phone=' + phone + '&ContractStart=' + timestamp + '&ContractEnd=' + rxtime;
...
function viewjob(job_id) {
    ...
    // Update spinner percentage (0-100)
    function updateSpinnerProgress(percentage, text = '') {
        const percentElement = document.getElementById('spinnerPercentage');
        const textElement = document.getElementById('spinnerProgressText');

        if (percentElement) {
            percentElement.textContent = `${Math.round(percentage)}%`;

            // Change color based on percentage
            const hue = Math.round(120 * percentage / 100); // 0-120 (red to green)
            percentElement.style.color = `hsl(${hue}, 100%, 65%)`;
        }
    ...
    function hideSpinner() {
        $('#customLoadingSpinner').remove();
    }

    // Show spinner with initial percentage
    showSpinner(0);
    ...
    const interval = setInterval(() => {
        progress += 1;
        updateSpinnerProgress(progress, `Loading ${progress} of 100`);

        if (progress >= 100) {
            clearInterval(interval);
            //setTimeout(hideSpinner, 500); // Hide after completion
        }
    }, 50);

	$('#viewForms').dialog('destroy');

    var WebAPI = 'https://webapi.systempest.com/api/maintenancejobinfo?MaintenanceID=' + job_id;
            ...
            $("<div id='viewForms'>").dialog({
                ...
                open: function () {
                    ...
                    var markup = "<div style='border-style: none; margin: 0px; padding: 0px 3px; overflow: auto;' class=''>" +
                        ...
                        "<thead>" +
                        "<tr>" +
                        "<th style='color: red; width: 50px;'><input type='checkbox' class='rowCheckbox' id='maintenancejob_selectAll' /></th>" +
                        ...
                        "<th style='color: blue;'>Admin Remarks</th>" +
                        ...
                        if (data[k] != undefined || data[k]) {
                            ...
                            if (value != "") {
                                for (j = 0; j < value.length; j++) {
                                    ...
                                    if (j + 1 == value.length) {
                                        markup += "<tr>" +
                                            "<td style='padding: 5px;' class='checkbox-cell'>" +
                                            "<div class='checkbox-wrapper'>" +
                                            "<input type='checkbox' class='rowCheckbox' id='maintenancejob_cb' />" +
                                            serialFormatter(k) +
                                            "</div>" +
                                            "</td>" +
                                            "<td style='padding: 5px;'>" +
                                            ...
                                            "</td>" +
                                            "<td style='padding: 5px;'>" +
                                            editactionFormatter(data[k].MaintenanceJobID) + " " + deleteactionFormatter(data[k].MaintenanceID, data[k].MaintenanceJobID) + " " + pdfactionFormatter(data[k].MaintenanceID, data[k].MaintenanceJobID) +
                                            "</td>" +
                                            "</tr>"
                                    }
                                }
                    ...
                    function deleteactionFormatter(maintenance_id ,maintenance_job_id) {
                        if(getSessionstorageValueRoleID <= 3){
                            return "<div>" + "<span>" + "<a href='javascript:deletemaintenancejob(" + maintenance_id + ',' + maintenance_job_id + ")' style='color:#458FD2;'>" + "Delete" + "</a>" + "</span>" + "</div>";
                        }else{
							return "<div></div>";
						}
                        //return "<button type='button' class='btn btn-primary btn-lg' data-toggle='modal' data-target='#MaintenanceJobForm'>Launch demo modal</button>";
                    ...
                    function pdfactionFormatter(maintenance_id, value) {
                        return "<div>" + "<span>" + "<a href='javascript:customemail(" + maintenance_id + ',' + value + ")' style='color:#458FD2;'>" + "Resend Report" + "</a>" + "</span>" + "</div>";
                        //return "<button type='button' class='btn btn-primary btn-lg' data-toggle='modal' data-target='#MaintenanceJobForm'>Launch demo modal</button>";
                    ...
                    // Fix for select all/deselect all (if you add a selectAll checkbox)
                    $('#maintenancejob_selectAll').change(function () {
                        $('#maintenancejobTable tbody input[type="checkbox"]').prop('checked', $(this).prop('checked'));
                    });

                },
                buttons: {
                    EnableMailButton: {
                        class: 'enableAllEmail',
                        text: 'Enable all Email Notification',
                        click: function () {
                            enableEmailNotification(data[0].MaintenanceID);
                        }
                    },
                    DisableMailButton: {
                        class: 'disableAllEmail',
                        text: 'Disable all Email Notification',
                        click: function () {
                    ...
                    ReferenceButton: {
                        ...
                        text: 'Update all Reference No',
                        click: function () {
                            updateReferenceNo(data[0].MaintenanceID, data[0].ReferenceNo);
                        }
                    },
                    ...
                    DeleteMaintenance: {
                        ...
                        click: function () {                         
                            ...
                            // Loop through each row with checked checkbox
                            $('#maintenancejobTable tbody tr').each(function () {
                                const checkbox = $(this).find('td:first-child input[type="checkbox"]');

                                if (checkbox.is(':checked')) {
                                    // Get the Maintenance Job ID from second column
                                    const jobId = $(this).find('td:nth-child(2)').text().trim();
                                    selectedIds.push(jobId);
                                }
                            });

                            //console.log("Selected Maintenance Job IDs:", selectedIds);
                            console.log("Selected IDs: " + selectedIds.join(', '));

                            // You can use selectedIds as needed (send to server, etc.)
                            ...
                            if (!selectedIds || !selectedIds.length) {
                                ...
                                // Create a custom notification div (bypassing niftyNoty if needed)
                                const notification = $('<div>')
                                    .html('Please Select a Maintenance Job!')
                                    .css({
                                        'position': 'fixed',
                                        'top': '20px',
                                        'right': '20px',
                                ...
                                deleteMaintenanceJob(data[0].MaintenanceID, selectedIds.join(', '));
                            }
                        }
    ...
    if (getSessionstorageValueRoleID > 3) {
        ...
        bootbox.confirm("<div style='color:black'>" + titleAreYouSureDelete + "</div>", function (result) {
            if (result) {
                ...
                var WebAPI = 'https://webapi.systempest.com/api/maintenanceinfo/' + data.MaintenanceID;

                var remarks = "Maintenance ID: " + data.MaintenanceID + "<br>" +
                            "Address: " + data.Address + "<br>" +
                            "Company: " + data.CusCompany + "<br>" +
                            "Person In Charge: " + data.PIC + "<br>" +
                            "User: " + sessionStorage.getItem('setSessionstorageValueUser') + "<br>" +
                            "Contract Start: " + datetimeFunction(data.ContractStart) + "<br>" +
                            "Contract End: " + datetimeFunction(data.ContractEnd) + "<br>" +
                            "Maintenance Interval: " + data.MaintenanceInterval + "<br>" +
                            "Status: Maintenance and Maintenance Job Deleted Successfully";
...
// SHOW spinner (with error handling)
function showLoadingSpinner() {
    try {
        const spinner = $('#customLoadingSpinner');
        ...
        console.log('Current z-index:', spinner.css('z-index'));
...
function showSpinner(percentage = 0) {
    // Remove any existing spinners first
    $('#customLoadingSpinner').remove();

    // Create new spinner with percentage display
    $('body').append(`
        <div id="customLoadingSpinner" style="
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        ">
            ...
            ">
                ...
                "></div>
                <div id="spinnerPercentage" style="
                    position: absolute;
                    top: 50%;
                    left: 50%;
            ...
            <div id="spinnerProgressText" style="
                font-size: 16px;
                margin-top: 10px;
                text-align: center;
                max-width: 80%;
...
function updateReferenceNo(maintenance_id, reference_no) {

    $("<div id='referenceForms'>").dialog({
        modal: true,
        title: "Update Reference No",
        width: '40%',
        position: 'fixed',
        height: 'auto',
        ...
        open: function () {
            ...
            var top = Math.max(jQuery(window).height() / 2 - jQuery(this)[0].offsetHeight / 2, 0) - 100;
            ...
            var markup = "<div class='col-sm-12'><div class='form-group'>" +
                "<label class='control-label'>Reference No</label>" +
                "<input id='update_reference_no' type='text' placeholder='Reference No' class='form-control'" +
                "</div></div>" 
            if (markup !== undefined || markup !== null) $(this).html(markup);
    
            $('#update_reference_no').val(reference_no);

        },
        buttons: {

            'Update Reference No': function () {
                
                if($('#update_reference_no').val() == ''){
                    alert('Reference No must not be empty')
                }else{
                    if (getSessionstorageValueLanguage == "en") {
                        bootbox.setDefaults({
                            /**
                    ...
                    bootbox.confirm("<div style='color:black'>" + "Are you sure you wish to update all reference no?" + "</div>", function (result) {
                        if (result) {
                            $('#LoadingForm').modal('show');
                            ...
                            var reference = {
                                ...
                                ReferenceNo: $('#update_reference_no').val(),
                            }
                            $('#viewForms').dialog('destroy');
                            $('#referenceForms').dialog('destroy');
                
                            var updateReferenceNo = 'https://webapi.systempest.com/api/maintenancejobreference?id=' + maintenance_id;
                            $.ajax({
                                url: updateReferenceNo,
                                type: "PUT",
                                async: false,
...
function enableEmailNotification(maintenance_id) {
    if (getSessionstorageValueLanguage == "en") {
        bootbox.setDefaults({
            /**
             * @optional String
    ...
    bootbox.confirm("<div style='color:black'>" + "Are you sure you wish to enable all email notification?" + "</div>", function (result) {
        if (result) {
            $('#LoadingForm').modal('show');
            ...
            var updateMaintenanceJob = 'https://webapi.systempest.com/api/maintenancejobalert?id=' + maintenance_id;
            $.ajax({
...
function deleteMaintenanceJob(maintenance_id, maintenancejob_id) {
    if (getSessionstorageValueLanguage == "en") {
        bootbox.setDefaults({
            /**
             * @optional String
    ...
    bootbox.confirm("<div style='color:black'>" + "Are you sure you wish to delete the selected Maintenance Job?" + "</div>", function (result) {
        if (result) {
            ...
            var maintenancejob = {
                MaintenanceJobID: maintenancejob_id,
                Param: maintenancejob_id
            };

            var bulkdeleteMaintenanceJob = 'https://webapi.systempest.com/api/maintenancejobbulkinfo';

            $.ajax({
                url: bulkdeleteMaintenanceJob,
                type: "POST",
                data: JSON.stringify(maintenancejob),
                crossDomain: true,
                contentType: 'application/json; charset=utf-8',
...
function addmaintenancejob(maintenance_id, maintenance_job_id) {
    ...
    console.log("https://webapi.systempest.com/api/maintenancejobinfo/" + maintenance_job_id)

    $.ajax({
        type: 'GET',
        url: "https://webapi.systempest.com/api/maintenancejobinfo/" + maintenance_job_id,
        async: false,
        data: {
            get_param: 'value'
        },
        ...
        success: function (value) {
            ...
            $("<div id='addForms'>").dialog({
                ...
                open: function () {
                    ...
                    $tableItemPest.on('check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table', function () {
                      ...
                      $('#deleteItemPest').on('click', function () {
                        ...
                        $buttonPest.click(function (e) {
                            ...
                            if ($("#newpests").val() != 0 || $("#newotherpests").val() != '') {
                                if ($("#newTreatments").val() != null || $("#newotherTreatments").val() != '') {
                                    insertDataPest();
                                } else {
                                    alert("Item cannot be Added - Select Treatment");
                                    //$.niftyNoty({
                    ...
                    $.getJSON('https://webapi.systempest.com/api/emailflaginfo', function (data) {
					...
					setTimeout(function(){ 
					  console.log(datetimeFormatter(value.Timestamp));
					}, 3000);
                        ...
                        if ($('#scheduledDate').val() == "") {
                            ...
                            if (moment(validDate).isValid()) {
                                if (moment(validDate).isSameOrAfter(moment(), "day")) {
                                    ...
                                    var total = $('#newpestsTreatment').bootstrapTable('getData').length;
                                    if ($('#newpestsTreatment').bootstrapTable('getData').length == 0) {
                                        ...
                                        if (result) {
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (json) {
                                                    ...
                                                    var addMaintenanceJob = 'https://webapi.systempest.com/api/maintenancejobinfo';
...
function editmaintenancejob(id) {
    ...
    $.ajax({
        type: 'GET',
        url: "https://webapi.systempest.com/api/maintenancejobinfo/" + id,
        async: false,
        data: {
            get_param: 'value'
        },
                ...
                open: function () {
                    ...
                    if (value.Flag == 0 && getSessionstorageValueRoleID > 4) {
                        ...
                        markup += "<tr>" +
                            ...
                            "</td>" +
                            "<td style='padding: 5px;'>" +
                            "<input id='scheduledDate' autocomplete='off' type='text' placeholder='Scheduled Date' class='form-control'>" +
                            "</td>" +
                            "<td style='padding: 5px;'>" +
                            "<select class='selectpicker' data-live-search='true' data-width='100%' data-style='btn-asiacomDropdown' id='maintenanceJobTechinician' name='maintenanceJobTechinician' data-table='searchable'></select>" +
                            "</td>" +
                            ...
                            "</td>" +
                            "<td style='padding: 5px;'>" +
                            "<textarea rows='16' class='form-control' placeholder='Admin Remarks' name='adminRemarks' id='adminRemarks' data-by-field='adminRemarks'></textarea>" +
                            "</td>" +
                            "<td style='padding: 5px;'>" +
                            "<input id='referenceno' type='text' placeholder='Reference No.' disabled='disabled' class='form-control'>" +
                            "</td>" +
                            ...
                            "<input type='checkbox' id='issent'></input>"+
                        "</td>" +
                        "<td style='padding: 5px;'>" +
                        ...
                        $('#adminRemarks').val(value.AdminRemarks);
                ...
                buttons: {
                    ...
                    'Update Maintenance Job': function () {
                        if ($('#scheduledDate').val() == "") {
                            ...
                            // Create a custom notification div (bypassing niftyNoty if needed)
                            const notification = $('<div>')
                                .html('Warning: Pls. select date from datepicker! Date must not be empty.')
                                .css({
                                    'position': 'fixed',
                                    'top': '20px',
                                    'right': '20px',
                            ...
                            if (moment(validDate).isValid()) {
                                ...
                                //    alert("Warning : Scheduled Date cannot be before Today's Date.");
                                        ...
                                        if (result) {
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (json) {
                                                    ...
                                                    var updateMaintenanceJob = 'https://webapi.systempest.com/api/maintenancejobinfo?id=' + maintenancejob.MaintenanceJobID;
                                                    var maintenanceFinalID = value.MaintenanceID;
                                                    $.ajax({
                                                        url: updateMaintenanceJob,
                                                        type: "PUT",
                                                        ...
                                                        success: function (res) {
                                                            ...
                                                            "Alert Date: " + datetimeFunction(maintenancejob.AlertDate) + "<br>" +
                                                            "Scheduled Date: " + datetimeFunction(maintenancejob.Timestamp) + "<br>" +                                                        
                                                            "Status: Maintenance Job Updated Successfully";
                                                          ...
                                                          $.ajax({
                                                            ...
                                                            $("#editForms").dialog("destroy");                                                         
                                                            viewjob(maintenanceFinalID);
                                                            editmaintenancejob(maintenancejob.MaintenanceJobID);
...
function deletemaintenancejob(maintenance_id, maintenance_job_id) {
    ...
    bootbox.confirm("<div style='color:black'>" + "Are you sure you wish to delete the maintenance job?" + "</div>", function (result) {
        if (result) {
            
            $('#viewForms').dialog('destroy');

            var remarks = "Maintenance Job ID: " + maintenance_job_id  + "<br>" +
            "Maintenance ID: " + maintenance_id + "<br>" +
            "User: " + sessionStorage.getItem('setSessionstorageValueUser') + "<br>" +                                                    
            "Status: Maintenance Job Deleted Successfully";
            ...
            var jobs = JSON.stringify({ 'MaintenanceJobID': maintenance_job_id });
                var WebAPI = 'https://webapi.systempest.com/api/removemaintenancejobinfo?id=' + maintenance_job_id;
...
function previewpdf(maintenance_id, id) {
    ...
    $.ajax({
        ...
        success: function (value) {
            ...
            if (value.Flag == 0) {
                    ...
                    open: function () {
                        ...
                        if (value.Flag == 0 && getSessionstorageValueRoleID > 4) {
                            ...
                            markup += "<tr>" +
                                ...
                                "</td>" +
                                "<td style='padding: 5px;'>" +
                                "<input id='scheduledDate' autocomplete='off' type='text' placeholder='Scheduled Date' class='form-control' disabled>" +
                                "</td>" +
                                "<td style='padding: 5px;'>" +
                                "<select class='selectpicker' data-live-search='true' data-width='100%' data-style='btn-asiacomDropdown' id='maintenanceJobTechinician' name='maintenanceJobTechinician' data-table='searchable' disabled></select>" +
                                "</td>" +
                                "<td style='padding: 5px;'>" +
                                "<textarea rows='16' class='form-control' placeholder='Technicians' name='maintechnicians' id='maintechnicians' data-by-field='maintechnicians' disabled></textarea>" +
                                "</td>" +
                                "<td style='padding: 5px;'>" +
                                "<textarea rows='16' class='form-control' placeholder='Cancel Remarks' name='cancelRemarks' id='cancelRemarks' data-by-field='cancelRemarks'></textarea>" +
                                "</td>" +
                                ...
                                "</td>" +
                                "<td style='padding: 5px;' align='center'>" +
                                "<input class='hidden' type='checkbox' id='issent' disabled></input>" +
                                "<input type='text' id='email'></input>" +
                                "</td>" +
                                "<td style='padding: 5px;'>" +
                                "<select id='status' class='selectpicker form-control' data-width='100%' data-style='btn-asiacomDropdown' disabled></select>" +
                                "</td>" +
                                "</tr>"
                            ...
                            $('#pest').val(pestFormatter(value.Pest, value.MaintenanceJobID));
                        ...
                        function pestFormatter(value, maintenancejobID) {
                            ...
                            if (value != "") {
                                ...
                                //strRtn += '<br />' +
                                //    '<div class="form-group pull-right">' +
                                //    "<span>" + "<a href='javascript:editpest(" + JSON.stringify(value) + ")' style='color:#458FD2;' disabled>" + "Edit" + "</a>" + "</span>" +
                                //    '</div>';
                        ...
                        'Resend Report': function () {
                            if ($('#scheduledDate').val() == "") {
                                ...
                                if (moment(validDate).isValid()) {
                                    ...
                                    bootbox.confirm("<div style='color:black'>" + "Are you sure you wish to resend report?" + "</div>", function (result) {
                                        if (result) {
                                            $('#viewForms').dialog('destroy');
                                            $.niftyNoty({
                                                type: 'success',
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (json) {
                                                    ...
                                                    var maintenancejob = {
                                                        ...
                                                        Technician: $("#maintechnicians").val(),
                                                        Email: $("#email").val(),
                                                        PestTreatment: $("#pest").val()
                                                    }

                                                    console.log('Maintenance Job Array : ' + JSON.stringify(maintenancejob));

                                                    var emailMaintenanceJob = 'https://webapi.systempest.com/api/emailmaintenancejob';
                                                    var maintenanceFinalID = value.MaintenanceID;
                                                    $.ajax({
                                                        url: emailMaintenanceJob,
                                                        type: "POST",
                                                        //async: false,
                                                        data: JSON.stringify(maintenancejob),
                                                        crossDomain: true,
                                                        ...
                                                        success: function (res) {
                                                            ...
                                                            var remarks = "Maintenance Job ID: " + maintenancejob.MaintenanceJobID + "<br>" +
                                                                ...
                                                                "Alert Date: " + datetimeFunction(maintenancejob.AlertDate) + "<br>" +
                                                                "Scheduled Date: " + datetimeFunction(maintenancejob.Timestamp) + "<br>" +
                                                                "Status: [Resend Report] Successfully";
                ...
                alert('Job must be "Completed" to resend the report')
...
function customemail(maintenance_id, id) {
                                        ...
                                        if (result) {
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (json) {
                                                    ...
                                                    var emailMaintenanceJob = 'https://webapi.systempest.com/api/customemail';
                                                    var maintenanceFinalID = value.MaintenanceID;
                                                    $.ajax({
                                                        url: emailMaintenanceJob,
                                                        type: "POST",
            ...
            'Submit': function () {
                ...
                if ($('#newpestsTreatment').bootstrapTable('getData').length == 0) {
                    ...
                    if (result) {
                        ...
                        var deletePestTreatment = 'https://webapi.systempest.com/api/deletemainpesttreatment';
            ...
            'Update Maintenance': function () {
                ...
                bootbox.confirm("<div style='color:black'>" + "WARNING: Maintenance Jobs will recalculate once you update the job. Proceed?" + "</div>", function (result) {
                    if (result) {
                        ...
                        $.ajax({
                            url: "https://webapi.systempest.com/api/maintenanceinfo?id=" + maintenance.MaintenanceID,
                            type: "PUT",
                            async: false,
                            data: JSON.stringify(maintenance),
                            crossDomain: true,
                            ...
                            success: function (maintenance) {
                                ...
                                $.ajax({
                                    url: "https://webapi.systempest.com/api/maintenancejobinfoex/" + $('#maintenanceID').val(),
                                    type: "GET",
                                    crossDomain: true,
                                    contentType: 'application/json; charset=utf-8',
                                    xhrFields: {
                                    ...
                                    success: function (res) {
                                        ...
                                        if (res.MaintenanceID > 0) {
                                            ...
                                            $.ajax({
                                                url: 'https://webapi.systempest.com/api/maintenancejobinfo/' + $('#maintenanceID').val(),
                                                type: "Delete",
                                                async: false,
...
function loadMaintenance(job_id) {
	...
	bootbox.setDefaults({
	 ...
	 bootbox.confirm("<div style='color:black'>" + titleAreYouSure + "</br></br>"+ "Warning: This action might affect the existing Maintenance Jobs. Please add in Driver and Pests & Treatments" +"</div>", function (result) {
    ...
    $.ajax({
        ...
        success: function (value) {
            ...
            var WebAPI = 'https://webapi.systempest.com/api/maintenanceinfo/' + value[0].MaintenanceID;
            $.ajax({
                type: 'GET',
                url: WebAPI,
                async: false,
                ...
                dataType: 'json',
                success: function (value) {
					$('#jobTechnician').prop("disabled", false);
					$('#contract_start_html').prop("disabled", false);
					$('#contract_end_html').prop("disabled", false);
					$('#maintenance_interval_html').prop("disabled", false);
					$('#notification_interval_html').prop("disabled", false);
					$('#isSent').prop("disabled", false);
                    $.niftyNoty({
                        type: 'danger',
                        icon: 'fa fa-check',
...
function loadMaintenanceInfo(job_id) {
    ...
    $('#LoadingForm').modal('show');
    var WebAPI = 'https://webapi.systempest.com/api/maintenanceinfo/' + job_id;
    $.ajax({
        type: 'GET',
        url: WebAPI,
        async: false,
        ...
        success: function (value) {
            ...
            $('#jobTechnician').prop("disabled", true);
			$('#contract_start_html').prop("disabled", true);
			$('#contract_end_html').prop("disabled", true);
			$('#maintenance_interval_html').prop("disabled", true);
			$('#notification_interval_html').prop("disabled", true);
			
			$('#isSent').prop("disabled", true);
...
$(function () {
    ...
    $('#companyName').autocomplete({
        source: function (request, response) {
            $.ajax({
                url: 'https://webapi.systempest.com/api/autocompletecompany?CompanyID=' + getSessionstorageValueCompanyID + '&CusCompany=' + $("#companyName").val(),
                type: 'GET',
                cache: false,
                data: {
    ...
    $('#jobDestination').autocomplete({
        source: function (request, response) {
            $.ajax({
                url: 'https://webapi.systempest.com/api/autocompleteaddress?CompanyID=' + getSessionstorageValueCompanyID + '&Address=' + $("#jobDestination").val(),
                type: 'GET',
                cache: false,
                data: {
...
setTimeout(function () {
    $('#pageLoadingForm').modal('hide');
}, 500);

//No Network Coverage Alert
setTimeout(function () {
    if ($('#pageLoadingForm').is(":visible")) {
        $('#pageLoadingForm').modal('hide');
...
function ExportToExcel(type, fn, dl) {
	var elt = document.getElementById('jobsCreated');
    var wb = XLSX.utils.table_to_book(elt, { sheet: "sheet1" });
    return dl ?
		XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }) :
        XLSX.writeFile(wb, fn || ('MaintenanceJob.' + (type || 'xlsx')));
}
