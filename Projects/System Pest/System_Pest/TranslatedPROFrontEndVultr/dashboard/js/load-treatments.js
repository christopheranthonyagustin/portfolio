...
$(document).ready(function () {
    ...
    $("#submit-filter").click(function () {
        ...
        $('#treatments-editable').bootstrapTable('destroy');

        var WebAPI = "https://webapi.systempest.com/api/treatmentinfo";

        $('#treatments-editable').bootstrapTable({
            idField: 'id',
            url: WebAPI,
            columns: [{
                field: 'state',
            ...
            onCheck: function (row) {
                ...
                var treatmentID = $('#treatmentID').val(row.TreatmentID);
                var treatmentDesc = $('#treatmentEN').val(row.TreatmentDesc);
                $(".selectpicker").selectpicker('refresh');
...
$(document).ready(function() {
	...
	$('#treatments-editable').bootstrapTable({
		...
		onCheck: function (row) {
            ...
            if (row.TreatmentID >= 1 && row.TreatmentID <= 42) {

                $("#toggle-aside").show();
                $("toggle-aside2").hide();
                clearInputField();
                $('#treatment-confirm-deleteEN').prop('disabled', true);
                $('#treatments-editable').bootstrapTable('refresh');

                $.niftyNoty({
                    type: 'danger',
                    icon: 'fa fa-times',
                    message: 'Default Treatments not allowed to edit',
                    container: 'floating',
                    timer: 3000
                });
...
//Delete Row
var $table = $('#treatments-editable'), $remove = $('#treatment-confirm-delete'), $removeEN = $('#treatment-confirm-deleteEN');
...
// =================================================================
function treatmentsFormatter(value, row) {
	return '<a href="#" class="btn-link" > Treatment #' + value + '</a>';
}
...
$(function () {
    ...
    $('.edit-treatmentEN').click(function () {
...
if (getSessionstorageValueLanguage == "en") {
    ...
    $("#next").text("Next");
    $("#submit-treatmentEN").text("Submit");
    $("#resellerName").text("Reseller");
    $("#enableReport").text("Enable Report");
    $("#clickLogoutEN").text("OK");
    $("#treatment-confirm-deleteEN").text("Delete");
    $("#TreatmentsExportWordEN").text("Word");
    $("#TreatmentsExportPDFEN").text("PDF");
    $("#TreatmentsExportExcelEN").text("Excel");
