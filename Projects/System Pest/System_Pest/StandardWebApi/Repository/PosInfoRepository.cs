...
namespace FMSWebApi.Repository
{
    public class PosInfoRepository : IPosRepository
    {
        ...
        public PosInfo Add(PosInfo item)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    //check for Location
                    if (currAsset.CompanyID == 1)
                    {
                        item.Location = "Singapore";
                        //GetLocation(currAsset, item, null);
                        //Logger.LogEvent("Count location: " + item.Location.Length.ToString(), System.Diagnostics.EventLogEntryType.Information);
                        //if (item.Location.Length == 0 || item.Location.Equals("Request exceeded")) VehTools.ApplyReverseGeocode(currAsset, item, null);
                    }
                    else if (currAsset.CompanyID == 2)
                    {
                        item.Location = "Vietnam";
                    }
                    else if (currAsset.CompanyID == 3)
                    {
        ...
        public bool UpdateLocation(PosInfo item)
        {
            bool retVal = false;
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "UPDATE positions SET loc = @Location WHERE pos_id = @PosID";
                        cmd.Prepare();
                        cmd.Parameters.AddWithValue("@Location", item.Location);
            ...
            {
                Logger.LogEvent(ex.Message + "-Update(PosLocationRepository)", System.Diagnostics.EventLogEntryType.Error);
            }
        ...
        private string GetLocArchive(double lat, double lon)
        {
            ...
            //Logger.LogEvent(string.Format("Query: {0} Lat: {1} Lon: {2}", query, lat, lon), System.Diagnostics.EventLogEntryType.Information);
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
