...
namespace FMSWebApi.Controllers
{
    ...
    public class TreatmentInfoController : ApiController
    {
        ...
        public TreatmentInfo PostTreatment([FromBody] TreatmentInfo currTreatment)
        {
            currTreatment = repository.Add(currTreatment);
            return currTreatment;
        }

        public bool PutTreatment(int id, [FromBody] TreatmentInfo currTreatment)
        {

            currTreatment.TreatmentID = id;
            if (repository.Update(currTreatment))
            {
                return true;
            }
            else
        ...
        {
            TreatmentInfo currTreatment = repository.Get(id);
            if (currTreatment == null)
            {
                throw new HttpResponseException(HttpStatusCode.NotFound);
            }
