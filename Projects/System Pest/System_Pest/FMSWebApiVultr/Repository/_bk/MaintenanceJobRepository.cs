...
namespace FMSWebApi.Repository
{
    public class MaintenanceJobRepository : IMaintenanceJobRepository
    {
        ...
        private static readonly string profilePath = @"C:\LibreOfficeProfile";
        ...
        public async Task<FMSWebApi.Models.MaintenanceJobInfo> CustomEmailAsync(FMSWebApi.Models.MaintenanceJobInfo _jobInfo)
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    try
                    {
                        bool ok = ConvertAndOptimizeReportPdf(targetFilePath, pdfFilePath, profilePath, logsDir);
                        if (ok)
                        {
                            ...
                            if (OptimizePdfWithGhostscript(pdfFilePath, optimizedPdf, logsDir))
                            {
             ...
             MainDocumentPart mainPart)
        {
            ...
            foreach (var para in paragraphs)
            {
                ...
                if (clientAvailable)
                {
                    ...
                    clientNote.Append(new Run(new RunProperties(new FontSize { Val = "16" }), new Text("(Signature over Printed Name)")));
                ...
                if (techAvailable)
                {
                    ...
                    var labelRun = new Run(
                        new RunProperties(new Underline { Val = UnderlineValues.Single }),
                        new Text(jobInfo?.Technician ?? "Technician")
                    );
                    techPara.Append(run);
                    ...
                    techNote.Append(new Run(new RunProperties(new FontSize { Val = "16" }), new Text("(Signature over Printed Name)")));
            ...
            string scope)
        {
            try
            {
                ...
                {
                    var newPara = new Paragraph(new Run(new Text(item)));
    ...
    string logsDir)
        ...
        private bool OptimizePdfWithGhostscript(string inputPdf, string outputPdf, string logsDir,
        ...
        private bool ConvertAndOptimizeReportPdf(string inputDocx, string finalPdfPath, string profilePath, string logsDir)
        {
        ...
        private string GetTemplateFileName(long templateId)
        {
            try
            {
                string templatesPath = IOPath.Combine(HostingEnvironment.ApplicationPhysicalPath, "images", "custom_templates");

                if (!Directory.Exists(templatesPath))
                {
                    Logger.LogEvent("Templates folder not found: " + templatesPath, System.Diagnostics.EventLogEntryType.Error);
                    return null;
                }

                // Template file name convention: templateId + extension
                string expectedFileName = templateId + ".docx"; // or ".xlsx" if needed
                string fullFilePath = IOPath.Combine(templatesPath, expectedFileName);

                if (!File.Exists(fullFilePath))
                {
                    Logger.LogEvent("Template not found: " + fullFilePath, System.Diagnostics.EventLogEntryType.Error);
                    return null;
                }
            ...
            {
                Logger.LogEvent($"Unable to retrieve template file for Template ID {templateId}: {ex.Message}",
        ...
        private Dictionary<string, string> BuildPlaceholdersFromModel(MaintenanceJobInfo job, string logsDir)
        {
            ...
            try
            {
                ...
                // Areas of Concern
                placeholders["{{AcInfo}}"] = job.AcInfo?.GeneralLocation ?? "";
                ...
                if (job.Pest != null && job.Pest.Any())
                {
                    placeholders["{{Pest}}"] = string.Join("\n",
                        job.Pest
                           .OrderBy(p => p.ItemNo)       // ensure correct order
                           .Select((p, i) => $"{i + 1}. {p.PestDesc}"));
                }
                else
                {
                    placeholders["{{Pest}}"] = "";
                ...
                if (job.Pest != null && job.Pest.Any())
                {
                    placeholders["{{Treatment}}"] = string.Join("\n",
                        job.Pest
                           ...
                           .Select((p, i) => $"{i + 1}. {p.TreatmentDesc}"));
                }
                else
                {
