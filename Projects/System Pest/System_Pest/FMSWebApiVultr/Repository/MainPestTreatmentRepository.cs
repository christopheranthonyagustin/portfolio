    ...
    public class MainPestTreatmentRepository : IMainPestTreatmentRepository
    {
        ...
        public IEnumerable<MainPestTreatmentInfo> GetAll()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT * FROM main_pests_treatment";
                cmd.Connection = conn;
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "INSERT INTO main_pests_treatment (maintenancejob_id, pest_desc, treatment_desc, item_no) " +
                                                               "VALUES (@MaintenanceJobID, @PestDesc, @TreatmentDesc, @ItemNo)";
                        //cmd.Prepare();
                        cmd.Parameters.AddWithValue("@MaintenanceJobID", currPestTreatment.MaintenanceJobID);
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "UPDATE main_pests_treatment SET maintenancejob_id = @MaintenanceJobID, pest_desc = @PestDesc, treatment_desc = @TreatmentDesc, item_no = @ItemNo" +
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "DELETE FROM main_pests_treatment WHERE maintenancejob_id = @MaintenanceJobID";
                        //cmd.Prepare();
