    ...
    public class DriverRepository : IDriverRepository
    {
        ...
        public IEnumerable<DriverInfo> GetAll()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT * FROM view_drivers";
                cmd.Connection = conn;
        ...
        public IEnumerable<DriverInfo> GetByCompany(DriverInfo param)
        {
            ...
            if (param.CompanyID == 1 || param.CompanyID == 3) query += " and company_id IN (1,3)";
                else if (param.CompanyID == 2) query += " and company_id = @CompanyID";
                ...
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.CommandText = "INSERT INTO drivers (driver_name, email, address, phone, birthdate, relative_phone, relative_name" +
                                ", rfid_id, rfid, company_id, bloodtype, last_tap, remarks, technician, created_date, modified_date, created_by, modified_by, created_byName, modified_byName, password) " +
                                "VALUES (@Name, @Email, @Address, @Phone, @DateOfBirth, @RelativePhone, @RelativeName, @RFIDID, @RFID, @CompanyID, @BloodType, @LastTap, @Remarks, @Technician"
                        ...
                        cmd.Parameters.AddWithValue("@Technician", currDriver.Technician);
                ...
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.CommandText = "UPDATE drivers SET driver_name = @Name, email = @Email, address = @Address, phone = @Phone, birthdate = @DateOfBirth, relative_phone = @RelativePhone, relative_name = @RelativeName, " +
                                            "rfid_id = @RFIDID, rfid = @RFID, company_id = @CompanyID, bloodtype = @BloodType, last_tap = @LastTap, remarks = @Remarks, technician = @Technician, " +
