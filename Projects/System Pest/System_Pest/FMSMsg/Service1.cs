...
namespace FMSMsg
{
    ...
    partial class Service1 : ServiceBase
    {
        ...
        {
            // log service start
            string msg = string.Format("{0} messaging service started TRACK.", Settings.Default.ProjectName);
            LogEvent(msg, false);
            ...
            try
            {
                ...
                m_strEmail = "support@asiacom.co.th";
                m_smsModem = new Modem(this);

                //[Dan:25Jun14] Implement testing base on config setting
                LogEvent(string.Format("Check SMS"), true);
                if (Settings.Default.IsSMS) SendSMS("98637670", msg);
                //if (Settings.Default.IsSMS) SendSMS("84901968", msg);
        ...
        private void TestEmail()
        {
            try
            {
                ...
                eMail.To.Add(new MailAddress("anthony.agustin@asiacom.co.th"));


                // set email content
                eMail.Subject = "TRACK-Email Test";
                eMail.Body = "Test For TRACK THAILAND Email Alert";


                // send email
                //SmtpClient smtp = new SmtpClient("webmail.asiacom.co.th");
                //smtp.DeliveryMethod = SmtpDeliveryMethod.Network;
                //smtp.Credentials = new NetworkCredential("track@asiacom.co.th", "Track2015");
        ...
        private void SendSMS(string phoneNo, string msg)
        {
            ...
            {
                LogEvent(string.Format("Send SMS"), true);
                // send sms
        ...
        private void SendAllEmail(List<MessageInfo> arrMessages, ref string strMessageIds, ref string strMsgFleetIds)
        {
            ...
            {
                LogEvent(String.Format("SendAllEmail: {0} Track Email", ex.Message), false);
            }
        }
        ...
        private void CheckService()
        {
            ...
            //{
            //    // reopen connection
            //    m_svcClient = new FmsWcfClient("BasicHttpBinding_IWlocate");
            //    m_svcClient.Open();
            //}
                        ...
                        try
                        {
                            ...
                            if (strPhone.StartsWith("+65") && (strStatus != "REC READ"))
                            {
                                ...
                                if ((arrFINDRegUsers.Count <= 0) && (arrFINDRegTags.Count <= 0))
                                {
                                    ...
                                    strMessage = string.Format("Mobile number not registered. Please contact TRACK FIND technical support.({0})",
                                        "Track");
                                }
                ...
                case "find":
                    {
                        ...
                        if (arrParams.Length > 1)
                        {
                            ...
                            {
                                // send asset location
                                strMessage = string.Format("Failed to locate asset(s). Please contact TRACK FIND technical support.");
                                if (m_smsModem.SendSms(strPhone, strMessage))
                                {
                                    // save message
        ...
        public bool SendEmail(MessageInfo message)
        {
            ...
            try
            {
                ...
                {
                    // set email content
                    eMail.Subject = string.Format("TRACK Notification - {0} ({1})",
